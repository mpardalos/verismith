...
fromSynthTool :: SynthTool -> SynthDescription
fromSynthTool (SynthTool synthType synthBin synthDesc synthOutput) =
  SynthDescription
    synthTypeStr
    (toTextIgnore <$> synthBin)
    (Just synthDesc)
    (Just $ toTextIgnore synthOutput)
  where
    synthTypeStr = case synthType of
      XST -> "xst"
      Vivado -> "vivado"
      Yosys -> "yosys"
      Quartus -> "quartus"
      QuartusLight -> "quartuslight"
...
defaultConfig =
  Config
    ...
    []
    [fromSynthTool defaultYosys, fromSynthTool defaultVivado]
  where
    defMod =
      ProbMod
        0 -- Drop Output
...
synthesiser =
  SynthDescription
    ...
    .= (synthBin :: SynthDescription -> Maybe Text)
    <*> Toml.dioptional (Toml.text "description")
    .= (synthDesc :: SynthDescription -> Maybe Text)
